...
export default function Favourites() {
	...
	const removeFromFavourites = (pokemonId) => {
		//The unique identifier of the Pokémon to be removed.
	...
	const filteredFavourites = favourites.filter(
		(pokemon) => pokemon.name.toLowerCase().startsWith(searchTerm.toLowerCase()) //Uses the startsWith method to filter Pokémon names that begin with the searchTerm entered by the user.
	)
					...
					{filteredFavourites.map((data) => {
						...
						return (
							<div key={data.id} className="pokemon">
								...
								<button onClick={() => removeFromFavourites(data.id)}>-</button>
							</div>
						)
					})}
				</div>
